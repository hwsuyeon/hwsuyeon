...
filedirfinal = './outputData/damage/damageNight/' # output file directory
...
for i in range(len(tots)):
    ...
    if ee[0] != 0 :
        ...
        gg = 0
        hh = 0
    ii = [str(tots[i])] + aa + bb + cc + [ff] + [gg] + [hh] + ee
    numb = list(range(1, 9, 1))
...
TJTJSHP = TJTJSHP.loc[TJTJSHP['USABILITY'].isin(Ls), :].reset_index(drop = True)
