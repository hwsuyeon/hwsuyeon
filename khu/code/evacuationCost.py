...
finpath = './outputData/evacuation/evacuationCost/' # output file path
...
start_nodes = list(routeTable['BuildId'])

end_nodes = []
end_nodes = [x+len(set(routeTable['BuildId'])) for x in list(range(len(set(routeTable['ShelId']))))*len(set(start_nodes))]


capacities = []
capacities =  [9999] * len(start_nodes)
    
unit_costs = [int(i) for i in list(routeTable['Length'])]

evaPop = building_intersect['POP'].tolist()
capPop = shelter_diff['capacity'].tolist()

del_capPop = []
p = 0

for i in range(len(capPop)):
    p = int(capPop[i] * (sum(evaPop) / sum(capPop)))
    del_capPop.append(p)

for j in range(len(del_capPop)):
    if sum(del_capPop) > sum(evaPop):
        del_capPop[j] = del_capPop[j] - 1
    elif sum(del_capPop) < sum(evaPop):
        del_capPop[j] = del_capPop[j] + 1

tmp_capPop = capPop[:]
diff_capeva = (sum(del_capPop) - sum(evaPop))

### Define an array of supplies at each node.
demPop = []
demPop[:] = [x for x in evaPop]
tmp_capPop[:] = [-1*x for x in del_capPop]
supplies = demPop + tmp_capPop
...
minTable = pd.DataFrame(columns = ['BuildId', 'ShelId', 'Length', 'Pop'])
for i in range(min_cost_flow.NumArcs()):
    if min_cost_flow.Flow(i) != 0:
        row = pd.DataFrame(data = [[min_cost_flow.Tail(i),
                                    min_cost_flow.Head(i),
                                    min_cost_flow.UnitCost(i),
                                    min_cost_flow.Flow(i)]],
                           columns = ['BuildId', 'ShelId', 'Length', 'Pop'])
        minTable = minTable.append(row)


for i in range(len(set(minTable['ShelId']))):
    minTable = minTable.replace(sorted(list(set(minTable['ShelId'])))[i],
                                list(set(routeTable['ShelId']))[i])


key = []
for i in range(len(minTable)):
    key.append(str(minTable.iloc[i, 0])+str(minTable.iloc[i, 1]))



minTable.loc[:, 'key'] = key
minTable = minTable.drop(columns=['BuildId', 'ShelId', 'Length'])
minTable
...
for i in range(len(routeTable)):
    key.append(str(routeTable.iloc[i, 0])+str(routeTable.iloc[i, 1]))

routeTable.loc[:, 'key'] = key
routeTable



routeTable = pd.merge(routeTable, minTable,
                      how='inner',
                      on = 'key')
routeTable = routeTable.drop(columns=['key'])
